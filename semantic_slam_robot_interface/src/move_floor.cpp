...
public:
	PlanActionServer() :
	as_(nh_, "/move_floor", boost::bind(&PlanActionServer::executeCB, this, _1), false)
	{
		as_.start();
	}
	...
	void executeCB(const semantic_slam_msgs::elevatorGoalConstPtr& goal)
	{
		...
		if(ok == 1)
		{
			printf("[DEBUG][%d] entered elevator .\n",__LINE__);
			//action complete, 
			result.result = 1;
			as_.setSucceeded(result);	
		}else
		{
			printf("[DEBUG][%d] Failed enter \n",__LINE__);
			//failed
			result.result = 0;
