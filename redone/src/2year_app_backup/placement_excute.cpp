...
int main(int argc, char** argv)
{
	...
	string s;
	if (ros::param::get("/placement_excute_node/db_ip", s))
	{
	...
	int port;
	if (ros::param::get("/placement_excute_node/db_port", port))
	{
	...
	if(socketConnect() < 0)
		return -1;
	RequestObjects(4);	//1= problem's object
	RecvXmlFromSocket();
	...
	if(socketConnect() < 0)
		return -1;
	RequestObjects(5);	//2= problem's init
	RecvXmlFromSocket();
	...
	if(socketConnect() < 0)
		return -1;
	RequestObjects(6);	//3= problem's goal
	RecvXmlFromSocket();
