...
vector<T_KNOWLEDGE_ITEM> Parser(char src[]);
...
void fileparseObject()
{
	string filename = "/home/msi1080/catkin_ws/output_xml.txt";
	...
	while(1) 
	{
		...
		if(linestart != string::npos)	
		{
			...
			string str2 = alltext.substr(start+1, next - start - 1);
			printf("substr : %s\n",str2.c_str());
...
void fileparseInit()
{
	string filename = "/home/msi1080/catkin_ws/request_init_output.txt";
	std::ifstream infile(filename.c_str());
	std::string line;
	int curr, next, start;
	std::string alltext((std::istreambuf_iterator<char>(infile)),std::istreambuf_iterator<char>());
			...
			if(linestart != string::npos)	
			{
				...
				while(1)
				{
					...
					if(startPos != string::npos)	
					{
						...
						//5.2 parse value
						startPos = str2.find("<uri>http://www.redone.org/plan#");
						if(startPos != string::npos)	
						{
							start = str2.find("#", startPos);
...
void fileparseGoal()
{
	string filename = "/home/msi1080/catkin_ws/request_goal_output.txt";
	std::ifstream infile(filename.c_str());
	std::string line;
	int curr, next, start;
	std::string alltext((std::istreambuf_iterator<char>(infile)),std::istreambuf_iterator<char>());
...
int socketConnect()
{
	...
	serv_addr.sin_addr.s_addr = inet_addr("192.168.10.103");
