#include "rosplan_interface_movebase/RPGotoCheckpoint.h"

/* The implementation of RPMoveBase.h */
namespace KCL_rosplan {

	/* constructor */
	RPGotoCheckpoint::RPGotoCheckpoint(ros::NodeHandle &nh, std::string &actionserver)
	...
	bool RPGotoCheckpoint::concreteCallback(const rosplan_dispatch_msgs::ActionDispatch::ConstPtr& msg) 
	{
		...
		for(size_t i=0; i<msg->parameters.size(); i++) 
		{
			...
			// if(0==msg->parameters[i].key.compare("to") || 0==msg->parameters[i].key.compare("obj")) 
			// {
		...
		semantic_slam_msgs::goto_checkpointGoal goal;
	...
	int main(int argc, char **argv) {
		...
		ros::init(argc, argv, "rosplan_interface_semantic_slam_robot");
		ros::NodeHandle nh("~");
		...
		KCL_rosplan::RPGotoCheckpoint rpmb(nh, actionserver);
